    ...
    const provider = new GoogleAuthProvider();
     ...
     try {
           ...
           if(user){
            ...
            // Note: getIdToken() directly gets the string, slightly simpler if you don't need other info from IdTokenResult
            const IdTokenResultObject : IdTokenResult = await user.getIdTokenResult();
            console.log('IdTokenResult Object:', IdTokenResultObject); // Keep for debugging if partner used it
            ...
            // 3 give backend the user token at /api/auth/login endpoint
            // Note: Hardcoded URL. Plan is to use environment variables (Developer C task).
            const BackendResponse = await fetch('/api/auth/login' , { // TODO: Use `${import.meta.env.VITE_BACKEND_API_URL}/auth/login`
              method: 'POST',
              headers: {
                'Content-Type' : 'application/json',
              },
              body: JSON.stringify({ requestBody }), // Sending partner's structure: { requestBody: { idToken: ... } }
